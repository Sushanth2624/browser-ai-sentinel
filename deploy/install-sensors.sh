#!/usr/bin/env bash
# Idempotent install for the two Phase 2 sensor services. Root-only (packet capture). Mirrors
...
for svc in bas-zeek bas-suricata; do
...
$ok || exit 1
echo "Both sensors running. Logs: $REPO_ROOT/sensor/logs/{zeek,suricata}/"
